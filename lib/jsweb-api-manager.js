...
function parse_params(uri_with_tags) {
    ...
    result.pattern = '^' + result.pattern + '$';
    result.pattern = result.pattern.replace(/\/index/, '');
...
function API_manager(jw) {
    ...
    this.params_apis = [];
    this.handles     = {};
    this.proxy_handles={};                          // add by ramon 0124 for proxy
...
API_manager.prototype.is_ws_uri = function (uri) {
    if (!/\.js$/.test(uri)) { return false; }
    if (/\.\#[^\/.]*\.js$/.test(uri)) { return false; }
    if (/.*_flymake\.js$/.test(uri)) { return false; }
    var tags = uri.match(/(\([^\/.]+\))/g);
    if (!tags) { return false; };
    tags = tags.join().replace(/[\(\)]/g, ';').split(';');
    if (tags.indexOf('ws') < 0) {
        return false;
    }
    return true;
}

API_manager.prototype.parse_ws_api = function (file) {
    var uri = file.slice(this.jw.root.length);
    uri  = uri.replace(/\.js$/, '').replace(/\(.*?\)/g, '');
    var api = {
        file    : file,
        type    : 'ws',
	uri     : uri,
...
API_manager.prototype.parse_api = function (file) {
    var uri = file.slice(this.jw.root.length);
    if (this.is_ws_uri(uri)) {
        return this.parse_ws_api(file);
    }
    ...
    if (is_param_api(uri)) {
        ...
        if ( methods.indexOf("PROXY") > -1 ) {          // add by ramon 0124 for proxy
            api.type    = 'params_proxy';
        } else {
            api.type    = 'params';
        }
...
API_manager.prototype.unload_api = function (file) {
    ...
    } else if (api.type === 'params_proxy') {                   // add by ramon 0125 for proxy
        for (var i = 0; i < this.params_apis.length; i++) {
            var api = this.params_apis[i];
            if (api.file === file) {
                this.params_apis.slice(i);
    ...
    } else if (api.type === 'regular_proxy') {
            delete this.proxyHandles[api.uri];
            console.log("[DELETE]: \t[%s]\t"
                        + this.jw.port.protocol + "://%s:%s%s",
                        method, this.jw.hostname,
                        this.jw.port.port, api.uri);
...
API_manager.prototype.load_api = function (filename) {
    ...
    try {
        ...
        if ( (api.type.indexOf('proxy') === -1 ) &&         //modify by ramon 0124 for proxy
    ...
    if (api.type === 'regular') {
        ...
        console.log("[LOADED]: \t[%s]\t"
                    ...
                    api.methods, this.jw.hostname,
                    this.jw.port.port, api.pattern);
    } else if (api.type === 'regular_proxy') {                        // add by ramon 0124 for proxy
        let method = "proxy";
        console.log("[LOADED]: \t[%s]\t"
                        + this.jw.port.protocol + "://%s:%s%s",
                        method, this.jw.hostname,
                        this.jw.port.port, api.uri);
        if (typeof func === 'function'){                             // add by ramon 0220 for proxy
            this.proxy_handles[api.uri] = func;
        } else {
            this.proxy_handles[api.uri] = { proxy_target : func };
        }
    } else if (api.type === 'params_proxy') {                        // add by ramon 0125 for proxy
        api.proxy_target = func;
        this.params_apis.push(api);
        console.log("[LOADED]: \t[%s]\t"
                    + this.jw.port.protocol + "://%s:%s%s",
                    api.methods, this.jw.hostname,
        ...
        api.func = func;
        this.ws_apis[api.uri] = {func : func};
        console.log("[LOADED]: ws://%s:%s%s", this.jw.hostname, this.jw.port.port, api.uri);
    } else {
        console.error('Unsupport api type :', api);
    }
};
...
API_manager.prototype.on_request = function (req, res, next) {
    ...
    uris.push(uri + '/index');
    var handle = null;
    let proxy_handle = null;                                   // add by ramon 0124 for proxy
    let proxy_options= {};

    for (var i = 0; i < uris.length; i++) {
        handle = this.handles[req.method + '/' + uris[i]];
        if (handle) { break; }
        proxy_handle = this.proxy_handles[uris[i]];           // add by ramon 0124 for proxy
        if (proxy_handle) { break; }
    }
    if ( proxy_handle )  {                                   // add by ramon 0124 for proxy
        if (typeof proxy_handle === 'function') {            // add by ramon 0220 for proxy
            try {
                proxy_options = proxy_handle(this.jw);
            } catch (error) {
                console.error(error);
                proxy_options = null;
            }
        } else {
            proxy_options = {target:proxy_handle.proxy_target.target};
        }
        if ( proxy_options ) {
            req.jw.proxy.web(req, res , proxy_options );
        }
    } else if (handle) {
        if (req.jw.config.debug == 1) {
            console.log("[" + (new Date()).now() + "]:",
                        req.jw.hostname, req.jw.port.port,
                        req.method, req.path);
        }
        if (req.jw.config.debug == 2) {
        ...
        for (var i = 0; i < this.params_apis.length; i++) {
            ...
            for (var j = 0; j < uris.length; j++) {
                ...
                if (values
                  && (values.length === api.names.length)
                  && (api.methods.indexOf(req.method) >= 0)) {
                    req.params = {};
                    for (var i = 0; i < values.length; i++) {
                        req.params[api.names[i]] = values[i];
                    };
                    if ( api.methods.indexOf("PROXY") > -1 ) {                  //add by ramon 0215 for proxy
                        if (typeof api.proxy_target === 'function'){            //add by ramon 0220 for proxy
                            try {
                                proxy_options = api.proxy_target(this.jw);
                            } catch (error) {
                                console.error(error);
                                proxy_options = null;
                            }                            
                        } else {
                            proxy_options = {target:api.proxy_target.target};
                        }                       
                        console.log("[" + (new Date()).now() + "]:",
