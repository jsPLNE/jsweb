...
function API_manager(jw) {
    ...
    if (/.*\(api\)$/.test(this.jw.host.root)) {
        this.root_api = true;
    } else {
        this.root_api = false;
    }
    this.load_apis(this.jw.host.root);
...
API_manager.prototype.is_api_file = function (uri) {
    if (!/\.js$/.test(uri)) { return false; }
    if (/.*_flymake\.js$/.test(uri)) { return false; }
    if (this.root_api && uri.indexOf('/') < 0 ) {
        return true;
    };
...
API_manager.prototype.parse_api = function (file) {
    var uri = file.slice(this.jw.host.root.length);
    if (!this.is_api_file(uri)) {
        return null;
    }
...
API_manager.prototype.on_request = function (req, res, next) {
    ...
    console.log(key, this.handles);
    var handle = this.handles[key];
    if (handle) {
        if (this.debug) {
            console.log("[API call]: ",
                        this.jw.host.host, this.port.port,
                        req.method, req.path);
        }
        req.jw = this.jw;
        ...
        for (var i = 0; i < this.params_apis.length; i++) {
            ...
            console.log(uri, values, api);
            if (values && (values.length === api.names.length)) {
                console.log('bingo', api);
                req.params = {};
                for (var i = 0; i < values.length; i++) {
...
API_manager.prototype.on_file_changed = function (event, path, filename) {
    ...
    if (is_dir) {
        ...
        var api = parse_api_filename(this.root, path, file);
        if (!api) { return; }
        if (!isExecutable(path + "/" + file)) {
            for (var i = 0; i < api.methods.length; i++) {
                var method = api.methods[i];
                delete this.handles[method][api.path];
                this.remove_route(method, api.path);
                console.log("[DELETE]: [%s]\thttp://%s:%s%s",
                            method, this.jw.host.host,
                            this.port.port, api.path);
            }
            try {
            ...
            for (var i = 0; i < api.methods.length; i++) {
                ...
                console.log("[CHANGED]: [%s]\thttp://host:%s%s",
                            method, this.port.port, api.path);
            }
            return;
        }
    }
