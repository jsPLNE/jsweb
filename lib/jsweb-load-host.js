            ...
            if (fs.statSync(path).isDirectory()) {
                ...
                try {
                    ...
                    fs.watch(path, function (event, filename) {
                        require.uncache(this.path);
                        delete this.lib[this.key];
                        try {
                            var func = require(this.path);
                            if (typeof func === 'function') {
                                func = func.bind(this.jw);
                            }
                            this.lib[this.key] = func;
                            console.log("[jw.lib" + this.key_path + '.' + this.key + "] Reloaded ..");
                        } catch (e) {
...
function load_host(host, next) {
    ...
    let create_proxy_options = {};                                        //add by ramon 0124 for proxy
    jw.proxy = http_proxy.createProxyServer(create_proxy_options);
    jw.proxy.on('error', function (err, req, res) {
        res.writeHead(500, {
