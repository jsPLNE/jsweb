...
if (tags.indexOf('https') >= 0) {
    try {
        var f = base + "/config/jsweb.crt";
        console.log(f);
        options.cert = fs.readFileSync(f);
        options.key  = fs.readFileSync(base + "/config/jsweb.key");
        options.ca   = fs.readFileSync(base + "/config/jsweb.csr");
    } catch (error) {
...
for (var h = 0; h < hosts.length; h++) {
    ...
    jw.base = host.base;
    jw.app  = host.app;
    jw.app.set('views', host.base + '/views');
...
if (tags.indexOf('https') >= 0) {
    ...
    https.createServer(options, main).listen(port, function () {
        console.log('JSWEB listening HTTPS on port: %d for %s',
                    port, JSON.stringify(hosts));
    });
} else {
    main.listen(port, function () {
        console.log('JSWEB listening HTTP on port: %d for %s',
                    port, JSON.stringify(hosts));
    });
}
